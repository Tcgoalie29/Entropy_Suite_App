...
"""
    ...
    entropy_type_selection = st.sidebar.selectbox(
        "Select Entropy Measure",
        ("Sample Entropy (SampEn)", "Fuzzy Entropy (FuzzEn)", "Spectral Entropy (SpecEn)"),
...
- **Spectral Entropy (SpecEn)**: An entropy measure based on the frequency spectrum of the signal."""
    )
    ...
    entropy_type_mapping = {
        ...
        "Spectral Entropy (SpecEn)": "SpecEn"
    }
    entropy_type = entropy_type_mapping[entropy_type_selection]

    # Parameters specific to spectral entropy
    if entropy_type == "SpecEn":
        # No need for num_scales
        num_scales = 1
        N_spec = st.sidebar.number_input(
            "FFT Resolution (N)",
            min_value=1,
            value=1024,
            help="Resolution of the N-point FFT, an integer > 1."
        )
        Freqs = st.sidebar.text_input(
            "Frequency Band Edges (normalized)",
            value="0,1",
            help="Normalized band edge frequencies for calculating the band entropy (e.g., '0,0.5')."
        )
        Freqs = tuple(map(float, Freqs.strip().split(',')))
        Logx = st.sidebar.number_input(
            "Logarithm Base",
            min_value=0.0,
            value=np.exp(1),
            help="Logarithm base in the entropy formula."
        )
        Norm = st.sidebar.checkbox(
            "Normalize Entropy",
            value=True,
                        ...
                        if target_channel not in ch_names:
                            ...
                            with st.spinner('Calculating entropy...'):
                                if entropy_type != "SpecEn":
                                    ...
                                    Spec, BandEn = eh.SpecEn(ch_data, N=N_spec, Freqs=Freqs, Logx=Logx, Norm=Norm)
                                    entropy_df = pd.DataFrame(
                                        {"Channel": [target_channel], entropy_type: [Spec]}
                                    )
                                    ci_df = None  # CI not applicable
                            ...
                            if entropy_type != "SpecEn":
                                ...
                                fig.update_layout(
                                    ...
                                    yaxis=dict(
                                        title=f"{entropy_type}",
                                        tickfont=dict(color="#FFFFFF"),
                                        titlefont=dict(color="#FFFFFF")
                                    ),
                                    title_text=f"Spectral Entropy for {target_channel}",
                                    paper_bgcolor="#0E1117",
                                    plot_bgcolor="#0E1117",
                                )
                    ...
                    if data.empty:
                        ...
                        for idx, ch in enumerate(ch_names):
                            ...
                            if entropy_type != "SpecEn":
                                ...
                                Spec, BandEn = eh.SpecEn(ch_data, N=N_spec, Freqs=Freqs, Logx=Logx, Norm=Norm)
                                df_ch = pd.DataFrame({"Channel": [ch], entropy_type: [Spec]})
                        ...
                        if entropy_type != "SpecEn":
                            ...
                            fig.update_layout(
                                ...
                                title_text="Spectral Entropy Across Channels",
                                paper_bgcolor="#0E1117",
                                plot_bgcolor="#0E1117",
                            )
                ...
                try:
                    ...
                    for idx, (subject_name, data) in enumerate(all_data_dict.items()):
                        ...
                        if entropy_type != "SpecEn":
                            ...
                            avg_spec = np.mean(entropy_values)
                            avg_entropy = pd.DataFrame({'Subject': [subject_name], 'Channel': ['Average'], entropy_type: [avg_spec]})
                            entropy_list.append(avg_entropy)
                    ...
                    if entropy_type != "SpecEn":
                        ...
                        fig.update_layout(
                            ...
                            title_text="Spectral Entropy Across Subjects",
                            paper_bgcolor="#0E1117",
                            plot_bgcolor="#0E1117",
                        )
                ...
                try:
                    ...
                    for idx, (subject_name, data) in enumerate(all_data_dict.items()):
                        ...
                        if entropy_type != "SpecEn":
                            ...
                            Spec, BandEn = eh.SpecEn(ch_data, N=N_spec, Freqs=Freqs, Logx=Logx, Norm=Norm)
                            subject_spec_df = pd.DataFrame(
                                {"Subject": [subject_name], "Channel": [target_channel], entropy_type: [Spec]}
                            )
                            mse_single_channel_df = pd.concat([mse_single_channel_df, subject_spec_df], ignore_index=True)
                    ...
                    if entropy_type != "SpecEn":
                        ...
                        fig.update_layout(
                            ...
                            title_text=f"Spectral Entropy Across Subjects ({target_channel})",
                            paper_bgcolor="#0E1117",
                            plot_bgcolor="#0E1117",
                        )
        ...
        with st.expander(measure):
            ...
            if measure == "Sample Entropy (SampEn)":
                ...
                **Fuzzy Entropy** extends Sample Entropy by incorporating fuzzy membership functions, allowing for a smoother transition between similarity and dissimilarity of patterns. This makes it more robust to noise and short data lengths.
                """)
            elif measure == "Spectral Entropy (SpecEn)":
                st.write("""
                **Spectral Entropy** is an entropy measure based on the power spectral density (PSD) of the signal. It quantifies the complexity of the frequency content of the signal. A signal with a flat spectrum (white noise) has maximum spectral entropy, while a signal with concentrated power in a few frequencies has lower spectral entropy.
                """)
            ...
            if measure == "Sample Entropy (SampEn)":
                ...
                st.markdown("""
                - **FFT Resolution (`N`)**: Resolution of the N-point FFT, an integer > 1.
                - **Frequency Band Edges (`Freqs`)**: Normalized band edge frequencies for calculating the band entropy, values in range [0,1], where 1 corresponds to the Nyquist frequency.
                - **Logarithm Base (`Logx`)**: Logarithm base in the entropy formula, a positive scalar.
                - **Normalize Entropy (`Norm`)**: Option to normalize the entropy value.
                """)
            ...
            if measure == "Sample Entropy (SampEn)":
                ...
                st.write("""
                - Inouye, T., Shinosaki, K., Sakamoto, H., Toi, S., Ukai, S., Iyama, A., ... & Takeda, M. (1991). [Quantification of EEG irregularity by use of the entropy of the power spectrum](https://doi.org/10.1016/0013-4694(91)90126-U). *Electroencephalography and Clinical Neurophysiology*, 79(3), 204-210.
                """)
